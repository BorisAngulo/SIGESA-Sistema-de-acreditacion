...
const ReporteModalidades = () => {
  ...
  const getProgresoModalidades = () => {
    ...
    return modalidades
      .filter(m => selectedModalidad === 'todas' || m.id.toString() === selectedModalidad)
      .map(modalidad => {
  ...
  const getComparativaFacultades = () => {
    ...
    return facultades.slice(0, 6).map((facultad, index) => {
      const result = { facultad: facultad.codigo_facultad || facultad.nombre_facultad.substring(0, 3).toUpperCase() };
  ...
  const exportToExcel = () => {
    try {
      ...
      const tendenciasHeaders = ['Año', 'CEUB', 'ARCU-SUR', 'Total'];
      const tendenciasData = tendencias.map(t => [t.año, t.CEUB, t['ARCU-SUR'], t.total]);
      const wsTendencias = XLSX.utils.aoa_to_sheet([tendenciasHeaders, ...tendenciasData]);
    ...
    <div className="reporte-container" style={styles.container}>
                  ...
                  <td style={styles.tdCenter}>
                    <span style={{
                      ...
                      ...(modalidad.porcentaje_completado >= 75 ? styles.excelenteBadge :
                          modalidad.porcentaje_completado >= 50 ? styles.buenoSta :
                          styles.desarrolloBadge)
                    }}>
      ...
      <div style={styles.footer}>
        ...
        <p style={styles.footerDetails}>
          {selectedYear === 'todos' ? 'Vista General - Todos los años' : `Año ${selectedYear}`} • 
          {progresoModalidades.length} modalidades analizadas
        </p>
      </div>
    </div>
  );
